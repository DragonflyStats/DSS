---
title: "Reproducible Research Peer Assessment - Week 3"
...
```{r}
dataSplitNewFinal %>% group_by(disasterStringColumn,FATALITIES) %>% tally()

```

```{r}
dataSplitNewFinal %>% group_by(FATALITIES,INJURIES) %>% tally()
...
1. The total number of fatalities from 2006 and onwards for each type of disaster
2. The total number of injuries from 2006 and onwards for each type of disaster
3. The total amount of economic damage from 2006 and onwards for each type of disaster
...
```{r}
dataSplitNewFinal %>% head()
```

```{r}
dataSplitNewFinal %>% group_by(disasterStringColumn) %>% tally()
```
```{r}
dataSplitNewFinal %>% pull(FATALITIES) %>% head()
```
```{r}
dataSplitNewFinal %>% group_by(disasterStringColumn) %>%
  summarize(TotalFatalities = sum(FATALITIES)) %>%
  ggplot(aes(x=TotalFatalities,fill=disasterStringColumn)) + 
 geom_bar()
```
...
ggplot(dataSplitNewFinal,aes(x=FATALITIES)) + 
 geom_histogram() + 
  facet_wrap(.~disasterStringColumn)
...
```{r}
library(ggplot2)
ggplot(data = dataSplitNewAmounts, 
       aes(x=disasterStringColumn, y=amountDollarsInBillions))+
    #stat = "summary", fun.y = "sum", 
    geom_bar(fill = factor(disasterStringColumn))
